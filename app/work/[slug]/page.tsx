  ...
  return (
    <main className="bg-[var(--bg)] text-[color:var(--text-primary)] pt-14">
      ...
      <div className="max-w-case mx-auto px-8">
        ...
        {/* ─── Cover image ─── */}
        <div className="w-full rounded-card-mobile md:rounded-card overflow-hidden bg-[var(--surface)]">
          {project.coverImage ? (
        ...
        {sections.filter(s => !s.links?.length).map((section) => (
          <div key={section.title} className={`pt-14 border-t border-[var(--border-color)] ${section.links && section.links.length > 0 ? "pb-2" : "pb-6"}`}>
            ...
            {section.images.length === 1 ? (
              <div className="flex flex-col gap-3">
                <div className="w-full rounded-card-mobile md:rounded-card overflow-hidden bg-[var(--bg)]">
            ...
            {project.gallery.map((item, i) =>
              !Array.isArray(item) && typeof item === "object" && "stack" in item ? (
                ...
                <div key={i} className="grid grid-cols-2 gap-6">
                  {item.map((src) => (
                    <div key={src} className="w-full rounded-card-mobile md:rounded-card overflow-hidden bg-[var(--surface)]">
                      <LightboxImage src={src} className="w-full object-cover" />
                    </div>
                  ))}
                </div>
              ) : (
                <div key={item} className="w-full rounded-card-mobile md:rounded-card overflow-hidden bg-[var(--surface)]">
